...
ANTIBIOTIC_PATH = ('boundary', 'cytoplasm', 'nitrocefin')


def assert_antibiotic_conserved(data):
    '''Check that the total moles of antibiotic is constant over time.
    At every timepoint, sum the moles of antibiotic in the environment
    and in all the cells. Assert that the minimum and maximum value are
    close (using np.allclose).
    '''
    moles = [
        get_timepoint_total_moles(ANTIBIOTIC_PATH, data[time])
    ...
    assert np.allclose(min_moles, max_moles), (
        'Moles of antibiotic not constant: {}'.format(moles))
    return moles
